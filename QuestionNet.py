...
class QuestionNet:
	...
	def ansGen(self):
		...
		for out in self.outs:
			...
			answer='{}'.format(prediction[0])
			inc=model.most_similar(positive=['oxygen'], topn=3)
			
			allAns=[]

			allAns.append(answer)
			allAns.append(inc[0][0])
			allAns.append(inc[1][0])
			allAns.append(inc[2][0])
			
			print(answer[0])
			self.ansOut.append(allAns)
	...
	def makeJson(self):
		...
		incomes = { 'question': self.outs, 'answer': self.ansOut }
